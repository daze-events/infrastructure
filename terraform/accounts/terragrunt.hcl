...
locals {
  ...
  environment_vars = read_terragrunt_config(find_in_parent_folders("env.hcl", find_in_parent_folders("empty.hcl")))
  ...
  account_id   = local.common_vars.locals.accounts[local.account_name].id
...
%{for region in local.supported_regions_vars.locals.all_aws_regions}
provider "aws" {
  region = "${region}"
  alias  = "${replace(region, "-", "_")}"
  # Skip credential validation and account ID retrieval for disabled or restricted regions
  skip_credentials_validation = ${contains(coalesce(local.supported_regions_vars.locals.opt_in_regions, []), region) ? "false" : "true"}
  skip_requesting_account_id  = ${contains(coalesce(local.supported_regions_vars.locals.opt_in_regions, []), region) ? "false" : "true"}
}
%{endfor}
EOF
}
...
inputs = merge(
  ...
  local.supported_regions_vars.locals,
